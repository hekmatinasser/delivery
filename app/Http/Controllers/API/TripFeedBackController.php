...
class TripFeedBackController extends BaseController
{
    ...
    /**
     * @OA\Post(
     *     path="/api/v1/vehicle/trip/{code}/feedbacks",
     *     summary="Create trip feedback",
     *     description="Create feedback for a specific trip",
     *     tags={"Vehicle"},
     *     security={ {"sanctum": {} }},
     ...
     * )
     */
    public function createWithVehicle(Request $request, $code)
    {
        $user = User::find(Auth::id());
        $user->load('vehicle');
        if (!$user->vehicle) {
        ...
        $trip = Trip::where('trip_code', '=', $code)->where('vehicle_id', $user->vehicle->Id)->firstOrFail();
    ...
    /**
     * @OA\Put(
     *     path="/api/v1/vehicle/trip/{code}/feedbacks/{id}",
     *     summary="Update trip feedback",
     *     description="Update feedback for a specific trip",
     *     tags={"Vehicle"},
     *     security={ {"sanctum": {} }},
    ...
    public function updateWithVehicle(Request $request, $code, $id)
    {
        $user = User::find(Auth::id());
        $user->load('vehicle');
        if (!$user->vehicle) {
     ...
     * )
     */
    public function getWithVehicle(Request $request, $code)
    {
        $perPage = $request->input('per_page', 10);
        $page = $request->input('page', 1);
